...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->ClientSize = System::Drawing::Size(655, 458);
			this->DoubleBuffered = true;
			this->KeyPreview = true;
			this->Margin = System::Windows::Forms::Padding(2);
			this->MinimumSize = System::Drawing::Size(120, 105);
			this->Name = L"MyForm";
			this->Text = L"MyForm";
...
#pragma endregion
	...
	private: System::Void MyForm_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e) {
		...
		// отрисовка прямоугольника, в котором изображение
		Pen^ rectPen = gcnew Pen(Color::Black, 2);
		g->DrawRectangle(rectPen, left, top, Wx, Wy); // высота
		...
		for (int i = 0; i <= numYsect; i++) { // цикл по количеству линий
			float tmpYCoord_d = Wcy - i * gridStep_y; // нижняя координата 'y' у i-й диагональной линии
			float tmpYCoord_h = Wcy_work - i * gridStep_y; // координата 'y' у i-й вертикальной линии
														   // i-я диагональная линия
			g->DrawLine(gridPen, Wcx, tmpYCoord_d, Wcx_work, tmpYCoord_h);
			// i-я горизонтальная линия
			g->DrawLine(gridPen, Wcx_work, tmpYCoord_h, maxX, tmpYCoord_h);
			if (i > 0 && i < numXsect) // если линия не крайняя
									   // выводим текст в правой точке горизонтальной линии
				g->DrawString(tick_y.ToString("F4"), drawFont, drawBrush, maxX, tmpYCoord_h);
		...
		while (Wcy_w <= Wcy) { // пока не перебрали все прямоугольники
			...
			while (start.x < maxX) {
				...
				if (hasEnd) {
					y = f(x, z); // координата y конечной точки в мировых координатах
								 // вычисляем соответствующее значение в координатах экрана
					deltaY = (y - Vc_work.y) / V_work.y;
					end.y = Wcy_w - deltaY * Wy_work;
				}
				vec2 tmpEnd = end;
				bool visible = hasStart && hasEnd && clip(start, end, Wcx_w, Wcy_w - Wy_work, maxX, Wcy_w);
	...
	private: System::Void MyForm_KeyDown(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) {
		...
		case Keys::D:
			...
			break;
		case Keys::W:
			T = translate(0.f, 0.f, V_work.z / Wz_work) * T; // сдвиг графика назад на один пиксел
			break;
		case Keys::S:
			T = translate(0.f, 0.f, -V_work.z / Wz_work) * T; // сдвиг графика вперед на один пиксел
			break;
		case Keys::R:
		...
		case Keys::G:
			T = translate(-centerX, -centerY, -centerZ) * T; // перенос начала координат в центр
			T = scale(1.f * (1.f / 1.1), 1.f, 1.f) * T; // масштабирование относительно начала координат
			T = translate(centerX, centerY, centerZ) * T; // возврат позиции начала координат
			break;
		case Keys::Y:
			T = translate(-centerX, -centerY, -centerZ) * T; // перенос начала координат в центр
		...
		case Keys::H:
			T = translate(-centerX, -centerY, -centerZ) * T; // перенос начала координат в центр
			T = scale(1.f, 1.f * (1.f / 1.1), 1.1) * T; // масштабирование относительно начала координат
			T = translate(centerX, centerY, centerZ) * T; // возврат позиции начала координат
			break;
		case Keys::U:
			T = translate(-centerX, -centerY, -centerZ) * T; // перенос начала координат в центр
		...
		case Keys::J:
			T = translate(-centerX, -centerY, -centerZ) * T; // перенос начала координат в центр
			T = scale(1.f, 1.f, 1.f * (1.f / 1.1)) * T; // масштабирование относительно начала координат
			T = translate(centerX, centerY, centerZ) * T; // возврат позиции начала координат
			break;
		case Keys::Q:
			...
			break;
		case Keys::E:
			Wx_part = max(Wx_part * (1 / 1.1), 0.2);
			break;
		case Keys::C:
			Wy_part = min(Wy_part * 1.1, 0.9);
			break;
		case Keys::V:
			Wy_part = max(Wy_part * (1 / 1.1), 0.2);
			break;
		case Keys::D1:
			numXsect++;
			break;
