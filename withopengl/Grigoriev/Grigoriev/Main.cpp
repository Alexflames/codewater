...
//=============================================================================
	...
	glm::mat4 light2M; // модельная матрица для второго источника света
//=============================================================================
...
void cursorPosSave_callback(GLFWwindow* window, double xpos, double ypos) {
	if (LMBpressed) {
		...
		//// добавляем к преобразованиям переход к новой системе координат наблюдателя
		//T = glm::lookAt(glm::vec3(0), P, u) * T;
		glm::mat4 M = rotateP(0.1, glm::vec3(lastY - ypos, lastX - xpos, 0), glm::vec3(0, 0, -dist)); // матрица вращения относительно точки P
...
int main() {
	...
	glfwSetCursorPosCallback(window, cursorPosSave_callback);
	// назначение обработчика прокрутки мыши
	...
	vertexShader.clear();
	fragmentShader.clear();
	// Задекларируем uniform-переменные для новой шейдерной программы.
	...
	// Отрисовка окна и проверка событий
	while (!glfwWindowShouldClose(window)) { // пока окно window не должно закрыться
		glClearColor(0.2f, 0.4f, 0.3f, 1.0f); // назначаем цвет заливки
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);; // очищаем буфер заданным цветом и глубину максимальной
		...
		glDrawArrays(GL_LINES, 0, 26); // рисуем 13 отрезков второго источника света (в массиве 26 точек)
		glBindVertexArray(0); // отключаем вершинный массив
