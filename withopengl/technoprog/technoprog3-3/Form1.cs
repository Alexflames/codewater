...
namespace technoprog3_3
{
    public partial class Form1 : Form
    {
        ...
        {
            blackPen = new Pen(Color.Black, penSize); //подготавливаем перо для рисования
        }
        ...
        {//Обработчик события перемещения мыши по pictuteBox1
            if (e.Button == MouseButtons.Left && g != null) //Проверяем нажата ли левая кнопка мыши
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            {
                blackPen = new Pen(new HatchBrush(HatchStyle.Cross, color), penSize);
            }
            else if (radioButton2.Checked)
            {
                blackPen = new Pen(new SolidBrush(color), penSize);
            }
            else if (radioButton3.Checked)
            {
                blackPen = new Pen(new HatchBrush(HatchStyle.SolidDiamond, color, Color.White), penSize);
            }
            else
            {
                blackPen = new Pen(color, penSize);
            }
